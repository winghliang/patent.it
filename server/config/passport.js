...
var Prosecutor = require('../models/Prosecutor');
...
module.exports = function(passport) {
    ...
    passport.deserializeUser(function(id, done) {
        Inventor.findById(id, function(err, user) {
            if (user && (user.user_type == 'inventor')){        
                done(err, user);
            } else {
                Prosecutor.findById(id, function(err, user){
                    done(err, user);
                })
            }
        });
    ...
    function(req, email, password, done) { // callback with email and password from our form
        ...
        Inventor.findOne({ 'email' :  email }, function(err, user) {
            ...
            req.session.login_type = 'inventor';
            return done(null, user);
        });

    }));

    passport.use('prosecutor-local-reg', new LocalStrategy({
        usernameField : 'email',
        passwordField : 'password',
        passReqToCallback : true // 
    },
    function(req, email, password, done) { 
        ...
        process.nextTick(function() {
            ...
            // find a user whose email is the same as the forms email
            // we are checking to see if the user trying to login already exists
            Prosecutor.findOne({ 'email' :  email }, function(err, user) {
                // if there are any errors, return the error
                if (err)
                    return done(err);
                ...
                if (user) {
                    ...
                    // if there is no user with that email
                    // create the user
                    var newProsecutor = new Prosecutor();

                    // set the user's local credentials
                    newProsecutor.email = email;
                    ...
                    newProsecutor.save(function(err) {
                        console.log('adding prosecutor to database')
                        console.log('done is:', done)
                        if (err)
                            throw err;
                        return done(null, newProsecutor);
                    });
                }
    ...
    passport.use('prosecutor-local-login', new LocalStrategy({
        // by default, local strategy uses username and password, we will override with email
        usernameField : 'email',
        passwordField : 'password',
        passReqToCallback : true // allows us to pass back the entire request to the callback
